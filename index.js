...
module.exports = bot = async (bot, mek) => {
       ...
       return mek = mek.messages.all()[0]		
	       ...
	       const q = args.join(' ')		
               ...
               const pushname = mek.key.fromMe ? bot.user.name : conts.notify || conts.vname || conts.name || '-'
               ...
               //MESS		
                       ...
                       mess = {			
                               ...
                               error: {			
                                       stick: 'stickernya bukan yang itu:v',				
                                       Iv: 'Linknya error bre:v'			
                               },			
                               only: {				
                                       group: 'Khusus grup oi',			
                               }		
                       }
                       const isUrl = (url) => {
...
.sendMessage(from, teks, text, {
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        bot.sendMessage(to, media, MessageType.sticker,{quoted:mek})
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    bot.sendMessage(to, media, type, { quoted: mek, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
❏ *SELF-BOT* ❏`
    ...
    case 'sticktag':
            ...
            bot.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            } else {
         ...
         case 'totag':
            ...
            ini_buffer = fs.readFileSync(file)
            bot.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            bot.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
    ...
    case 'fitnah':
            ...
            var bot = gh.split("|")[2];
            bot.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
            break
    case 'settarget':
    ...
    case 'tomp3':
            ...
            bot.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
    case 'fast':
    ...
    case 'anime':
            ...
            bot.sendMessage(from,media,image,{quoted:mek,caption:'NIH'})
            }
            )
            .catch((error) => {
            console.log(error); 
    ...
    case 'kontak':
            ...
            + `TEL;type=CELL;type=VOICE;waid=${entah}:${phoneNum('+' + entah).getNumber('internasional')}\n`
            + 'END:VCARD'.trim()
            bot.sendMessage(from, {displayName: `${nah}`, vcard: vcard}, contact)
            break    
    case 'take':
    case 'colong':
...
.sendMessage(from, bur, text, { quoted: mek })
    ...
    case 'image':
 	...
 	case 'tiktok':
		...
		tik.ssstik(`${args[0]}`)
    		...
    		.then(async (a) => {
    		me = `*Title* : ${text}\n*Link* : ${a.data}`
		bot.sendMessage(from,{url:`${videonowm}`},video,{mimetype:'video/mp4',quoted:mek,caption:me})
		})
		})
    ...
    case 'tiktokaudio':
		...
		bot.sendMessage(from,{url:`${music}`},audio,{mimetype:'audio/mp4',filename : `${text}`,quoted:mek})
		})
     		.catch(e => console.log(e))
     		break
    case 'brainly':
			...
			bot.sendMessage(from, teks, text,{quoted:mek,detectLinks: false})                        
            })              
			break
    case 'ig':
	    ...
	    bot.sendMessage(from,igvdl,video,{mimetype:'video/mp4',quoted:mek,caption:'Nih'})
            } else if (ink.includes('.jpg')){
            const igpdl = await getBuffer(ink)
            bot.sendMessage(from,igpdl,image,{mimetype:'image/jpeg',quoted:mek,caption:'Nih'})
	    }
            }
	    })
	    break
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
           for ( let y of participants) {
             ...
             bot.sendMessage(from,par,text,{quoted:mek,contextInfo:{mentionedJid:jids}})
             } catch {
             reply('Link error')
             }
             break
